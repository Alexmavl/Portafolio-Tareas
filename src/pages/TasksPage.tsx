...
type Task = {
  ...
  link?: string | null;  // ruta interna
  repo?: string | null;  // enlace a GitHub
  desp?: string | null;  // despliegue externo
};
...
const TasksPage = () => {
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-gray-900 via-blue-900/20 to-teal-900/20 text-white p-6">
      {/* Elementos de fondo (decorativos, no capturan mouse) */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {tasksData.map((task) => (
          <div
            key={task.id}
            className="group relative bg-gray-800/90 backdrop-blur-sm rounded-2xl shadow-2xl p-6 border border-gray-700 hover:border-blue-500/50 transform transition-all duration-500 hover:scale-[1.02] hover:shadow-blue-500/20"
          >
            {/* Glow decorativo (no capta eventos) */}
            <div className="pointer-events-none absolute -inset-0.5 bg-gradient-to-r from-blue-600 via-teal-600 to-cyan-600 rounded-2xl blur opacity-0 group-hover:opacity-20 transition duration-300" />
